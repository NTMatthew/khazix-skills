...
description: |
  ...
  当用户想要系统性研究一个产品、公司、概念、技术或人物时使用。核心是双轴分析：纵轴追踪从诞生到当下的完整生命历程（以叙事故事呈现），横轴在当下时间截面上与竞品/同类进行系统性横向对比，最后交叉两条轴产出独到洞察。默认产出一份可继续编辑和复用的Markdown研究报告（可选再转PDF）。
  ...
  不要用于简单的名词解释（用户只是问"XX是什么"）、不要用于写作（那个用khazix-writer）、不要用于纯标题摘要生成（用wechat-title）。
---
...
你正在执行一次横纵分析法深度研究。最终产出一份**完整的Markdown研究报告**（默认交付格式）。

## 前置准备

### 环境准备

1. **确认输出目录**：先确认用户的目标输出目录。若用户明确指定，使用用户指定目录；若未指定，默认使用 `Inbox`。目录不存在时先创建。
2. **可选PDF能力（仅在用户明确需要时）**：本Skill自带 `scripts/md_to_pdf.py`（基于WeasyPrint），可将Markdown转为PDF。仅当用户明确要求PDF时再安装依赖：`pip install weasyprint markdown --break-system-packages`。
...
以下元素适合普通文章但不适合研究报告，需要克制：

- **过强的口语化**：报告可以有聊天感，但不要满篇「这玩意」「不是哥们」「太牛逼了」。偶尔点缀可以，但密度要比普通文章低很多。
- **去小标题化**：普通文章追求一口气顺下来不加小标题。研究报告不一样，1-3万字的内容如果没有清晰的结构和导航，读者会迷路。报告需要清晰的章节结构。
- **标点禁令可以放松**：普通文章禁用冒号和破折号。研究报告中可以正常使用，因为报告需要的是信息传达效率。但「」的使用习惯可以保留。
- **固定尾部**：不要加默认尾部。
...
## 第五步：输出Markdown报告（默认交付）

报告写完后，默认交付**Markdown文件**，并保存到用户指定目录（若未指定，默认 `Inbox`）。

### 输出流程

1. **先确认输出目录**
   - 用户已指定目录：直接使用（例如 `Inbox`）
   - 用户未指定目录：默认使用 `Inbox`
   - 若目录不存在：先创建目录
2. **完成Markdown稿件**：将完整报告保存为 `[研究对象]_横纵分析报告.md`
3. **写入目标目录**：将文件保存到 `[输出目录]/[研究对象]_横纵分析报告.md`
4. **交付时说明路径**：明确告诉用户Markdown文件的绝对或相对路径

### Markdown写作注意事项

- 第一行用 `# 标题` 作为报告标题
- 紧接标题后可用 `> 研究时间：... | 所属领域：... | 研究对象类型：...` 写元信息
...
Markdown文件命名为 `[研究对象名称]_横纵分析报告.md`，保存到用户指定目录（默认 `Inbox`）。

### 可选：当用户明确要求PDF时

仅在用户明确要求PDF时，才执行转换：

1. 安装依赖（如未安装）：`pip install weasyprint markdown --break-system-packages`
2. 运行脚本：
   ```bash
   python [skill目录]/scripts/md_to_pdf.py input.md output.pdf --title "研究对象名称" --author "数字生命卡兹克"
   ```
3. 默认同时保留Markdown源文件，不覆盖
